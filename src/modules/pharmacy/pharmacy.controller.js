...
export const getAllPharmacies = async (req, res) => {
    try {
        ...
        const data = await getPharmacyServices(filter);
        res.status(200).json({
            success: true,
            count: data.length,
            data
        ...
        res.status(500).json({
            success: false,
            message: 'ফার্মেসী ও ঔষধ দোকানের তালিকা আনতে ত্রুটি হয়েছে',
            error: error.message
        });
    }
};
...
export const getPharmacyById = async (req, res) => {
    try {
        const data = await getPharmacyServiceById(req.params.id);
        if (!data) {
            return res.status(404).json({
                success: false,
                message: 'ফার্মেসীর তথ্য পাওয়া যায়নি'
            });
        }
        res.status(200).json({
            success: true,
...
export const createPharmacy = async (req, res) => {
    try {
        const data = await createPharmacyService(req.body);
        res.status(201).json({
            success: true,
            message: 'ফার্মেসী যুক্ত করার আবেদন সফলভাবে জমা দেওয়া হয়েছে',
            data
        });
    } catch (error) {
        res.status(400).json({
            success: false,
            message: 'নতুন ফার্মেসী সংরক্ষণ করতে ব্যর্থ হয়েছে',
            error: error.message
        });
    }
};
...
export const updatePharmacy = async (req, res) => {
    try {
        const data = await updatePharmacyService(req.params.id, req.body);
        if (!data) {
            return res.status(404).json({
                success: false,
...
export const deletePharmacy = async (req, res) => {
    try {
        const data = await deletePharmacyService(req.params.id);
        if (!data) {
            return res.status(404).json({
                success: false,
                message: 'তথ্য পাওয়া যায়নি'
